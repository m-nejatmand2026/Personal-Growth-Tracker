...
The target runtime cascade has five responsibilities:
...
Two late compatibility sheets still exist while recovered real-device behavior is migrated to its proper owner: `functional-recovery.css` and `screenshot-recovery.css`. They are temporary debt, not new architectural layers. New presentation work must not expand their scope. Rules should move out as soon as the owning shell/module/composition sheet can carry them without regression; the end state deletes both files.

## Retired presentation generations

The following global layers are deleted and must not return:

- `ux-reset.css`
- `living-canvas.css`
- `figma-current.css`
- `figma-current-live.css`
- `figma-current-semantics.css`
- `product-polish.css`

Their useful behavior has either been removed as obsolete or moved to a canonical owner. Tests must protect user-visible behavior and ownership rather than requiring these historical filenames.

## Prohibited patterns

- Do not create another global `reset`, `recovery`, `current`, `v2`, `final`, `fix`, or `override` stylesheet to win the cascade.
- Do not restore the deleted Figma Current/Living Canvas/polish presentation generations.
...
`!important` is exceptional. It is acceptable for narrowly-scoped accessibility/reduced-motion invariants, modal/state isolation where historical declarations cannot safely win, and temporary compatibility rules with an explicit removal path. New shell and module styling should normally require none.
...
`tests/presentation-architecture.test.js` protects the current boundaries. It rejects all retired presentation generations, requires the canonical load order, caps the runtime stylesheet count at 31, ensures the shell owns desktop Explore geometry, and prevents shell specificity from escalating.
