...
test('Plan keeps capacity arithmetic factual and non-moralized',()=>{assert.match(plan,/Capacity is arithmetic, not a productivity score\./);assert.match(plan,/Plan chooses what matters next/);assert.match(plan,/Progress still records only what actually happened/);assert.match(plan,/Direction → Plan → time → Action → Progress/);assert.doesNotMatch(plan,/streak|points|level up|good job|bad day|failure score/i)});
