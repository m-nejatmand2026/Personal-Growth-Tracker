...
test('375px acceptance layer prevents horizontal shell overflow without imposing a desktop minimum width',()=>{assert.match(css,/@media \(max-width: 375px\)/);assert.match(css,/overflow-x:\s*clip/);assert.match(css,/min-inline-size:\s*0/);assert.match(css,/max-inline-size:\s*100%/);assert.match(css,/100dvh/);assert.doesNotMatch(css,/min-(?:inline-)?size:\s*(?:4\d\d|[5-9]\d\d|\d{4,})px/)});
