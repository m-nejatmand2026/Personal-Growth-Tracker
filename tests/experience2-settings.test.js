...
test('Experience 2 Settings is product-facing appearance data and help',()=>{assert.match(app,/renderSettings\s*,\s*bindSettings\s*,\s*applyPresentationPreferences/);assert.match(app,/function renderSettingsView\(version=renderVersion\)/);assert.match(app,/if\(current==='settings'\)\{renderSettingsView\(version\);return;\}/);assert.match(settings,/Appearance, data and help/);assert.match(settings,/Your data/);assert.match(settings,/Export my data/);assert.match(settings,/Open full tutorial/);assert.doesNotMatch(settings,/Preview 2|Experience selector|offline cache|Reset Experience/);assert.match(html,/\/experience\/2\/css\/settings\.css/);});

test('Settings preferences remain isolated while product export uses only the dedicated export boundary',()=>{assert.match(preferences,/PREFERENCE_PREFIX='growth-compass:preview2:e2:'/);assert.match(settings,/readPreference\('theme','dark'\)/);assert.match(settings,/writePreference\('theme',input\.value\)/);assert.match(settings,/writePreference\('motion',input\.value\)/);assert.doesNotMatch(settings,/localStorage\.clear\s*\(/);assert.match(settings,/fetch\('\/api\/export'/);assert.doesNotMatch(settings,/\/api\/v1\//);});
