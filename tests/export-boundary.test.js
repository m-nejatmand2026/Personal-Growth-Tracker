...
const legacyCompatibility = await readFile(
  new URL('../worker/compatibility/legacy-beta/export.js', import.meta.url),
  'utf8'
);
...
test('Legacy Beta export is explicit compatibility data only', () => {
  ...
  assert.match(source, /exportLegacyBeta/);
  assert.doesNotMatch(source, /\bFROM\s+(?:activities|sessions|energy_logs)\b/i);
  ...
  for (const legacyTable of [
    ...
    assert.match(
      legacyCompatibility,
      new RegExp(`\\bFROM\\s+${legacyTable}\\b`, 'i')
    );
  }
});

test('Legacy Beta export is isolated to the original default profile', () => {
  assert.match(legacyCompatibility, /DEFAULT_PROFILE_ID/);
  assert.match(legacyCompatibility, /profileId\s*!==\s*DEFAULT_PROFILE_ID/);
  assert.match(legacyCompatibility, /return EMPTY_LEGACY_EXPORT/);
});
