...
test('Runtime weekly direction comes from Plans and Progress rather than legacy targets',async()=>{assert.match(weekCompatibility,/plansContractV1/);assert.match(weekCompatibility,/progressContractV1/);assert.doesNotMatch(weekCompatibility,/weekly_targets|getTargets/);assert.doesNotMatch(bootstrap,/getTargets|momente_lessons|roadmap_items/);assert.equal(await exists(new URL('../worker/data/targets.js',import.meta.url)),false)});
...
test('Legacy target mutation is retired and Settings points planning back to Plan',()=>{assert.match(targetsRoute,/410/);assert.doesNotMatch(targetsRoute,/weekly_targets|UPDATE\s+weekly_targets/i);assert.doesNotMatch(settings,/\/api\/targets|data-target=|data-minimum=/);assert.match(settings,/managed in Plan/i)});
