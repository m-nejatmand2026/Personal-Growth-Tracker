...
async function exercise(page,browserName,viewport){
  ...
  await assertNoOverflow(page,`${browserName} ${viewport} direction page`);
  ...
  await assertNoOverflow(page,`${browserName} ${viewport} create dialog`);
  ...
  await assertNoOverflow(page,`${browserName} ${viewport} success dialog`);
  ...
  await tracking.locator('summary').click();assert.equal(await tracking.getAttribute('open'),'');
  ...
  const card=page.locator('.goal-card',{hasText:name});await card.waitFor({state:'visible'});assert.match(await card.innerText(),/Career/);assert.match(await card.innerText(),/This makes future choices easier\./);assert.match(await card.innerText(),/Make this direction actionable/);assert.match(await card.innerText(),/Plan next step/);
  await card.locator('[data-goal-open]').click();const detail=page.getByRole('dialog',{name});await detail.waitFor({state:'visible'});assert.match(await detail.innerText(),/Why this matters/);assert.match(await detail.innerText(),/Tracking/);assert.match(await detail.innerText(),/Next useful move/);assert.match(await detail.innerText(),/Plan next step/);
  ...
  await page.evaluate(()=>localStorage.setItem('growth-compass:preview2:e2:theme','light'));await page.reload({waitUntil:'domcontentloaded'});await page.locator('.direction-view').waitFor({state:'visible'});assert.equal(await page.locator('html').getAttribute('data-theme'),'light',`${browserName} ${viewport}: light preference must apply`);add=page.locator('[data-goal-new]').first();await add.click();dialog=page.getByRole('dialog',{name:'Where do you want to move?'});await dialog.waitFor({state:'visible'});const themeColors=await dialog.evaluate(node=>{const style=getComputedStyle(node);return{background:style.backgroundColor,text:style.color};});const expected=await page.evaluate(()=>{const probe=document.createElement('i');probe.style.cssText='position:fixed;visibility:hidden;background:var(--gc-surface);color:var(--gc-text)';document.body.appendChild(probe);const style=getComputedStyle(probe),result={background:style.backgroundColor,text:style.color};probe.remove();return result;});assert.equal(themeColors.background,expected.background,`${browserName} ${viewport}: Direction editor must use light semantic surface`);assert.equal(themeColors.text,expected.text,`${browserName} ${viewport}: Direction editor text must use light semantic text`);await page.keyboard.press('Escape');
}
