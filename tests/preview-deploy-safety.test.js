...
test('Quality tests the exact PR head SHA later supplied by workflow_run', () => {
  ...
  assert.match(quality, /\.artifacts\/test-logs\/unit\.log/);
  assert.match(quality, /\.artifacts\/test-logs\/integration\.log/);
  assert.match(quality, /\.artifacts\/test-logs\/browser\.log/);
