...
test('zero-data setup creates real Direction and one Today action in three resumable neutral steps',()=>{assert.match(onboardingV2,/sessionDraft=\{step:1,areaKey:''/);assert.match(onboardingV2,/const state=sessionDraft/);assert.match(onboardingV2,/duration:null/);assert.match(onboardingV2,/state\.step===2/);assert.match(onboardingV2,/state\.step===3/);assert.match(onboardingV2,/Nothing is selected for you/);assert.match(onboardingV2,/Not sure/);assert.match(onboardingV2,/goalsCapability\.create/);assert.match(onboardingV2,/activitiesCapability\.create/);assert.match(onboardingV2,/api\.post\('\/v1\/daily-plan'/);assert.match(onboardingV2,/source:'manual'/);assert.match(onboardingV2,/Routine setup can wait/);assert.doesNotMatch(onboardingV2,/cloud architecture|German|Frankfurt/i);});
