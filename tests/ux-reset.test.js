...
test('Product Rebuild layers override rejected Current presentation and accessibility remains last', () => {
...
test('Today puts daily action and capacity first without moving business logic into composition', () => {
  assert.match(today, /\$\{dailyPlanPanel\}[\s\S]*\$\{capacityCard\(capacityModel\)\}/);
  assert.match(today, /gc-today-more/);
...
test('Progress foregrounds factual evidence and keeps interpretation out', () => {
  ...
  assert.match(progress, /Evidence only here\. Interpretation belongs in Insights\./);
});

test('Insights shows evidence readiness first and keeps methodology on demand', () => {
  assert.match(insights, /<h2 id="insightsCurrentTitle">Insights<\/h2>/);
  assert.match(insights, /Evidence readiness/);
  assert.match(insights, /No defensible matched pattern yet/);
  assert.match(insights, /<details class="gc-insight-method">/);
});

test('mobile navigation exposes five primary destinations plus accessible More', () => {
  assert.match(indexHtml, /id="quickAddBtn"[\s\S]*?<b>Add<\/b>/);
  assert.match(indexHtml, /aria-label="Add activity"/);
  ...
  assert.match(rebuildCss, /\.topbar\{position:fixed!important/);
  assert.match(app, /function closeTopMore\(\)/);
});

test('desktop rail keeps navigation separate from explicit Add activity', () => {
  ...
  assert.match(indexHtml, /rail-log-btn[^>]*>[\s\S]*?Add activity<\/button>/);
  assert.match(rebuildCss, /\.rail-log-btn\{background:var\(--gc-brand\)!important/);
});

test('Journal uses a destination heading and preserves its privacy boundary', () => {
...
test('Logger semantic guidance remains available for assistive technology', () => {
  assert.match(accessibilityCss, /\.logger-mode-fieldset legend,\s*\.logger-mode-hint/);
});

test('Product Rebuild preserves touch floors and reduced motion', () => {
  assert.match(rebuildCss, /min-height:44px/);
