...
test('normal settings contains product settings, not preview diagnostics',()=>{assert.match(settings,/Appearance and help/);assert.match(settings,/Open full tutorial/);assert.doesNotMatch(settings,/Preview 2|offline cache|Experience selector|Reset Experience 2|CACHE_PREFIX|PREFERENCE_PREFIX/);});
