...
test('primary navigation keeps five mobile destinations and secondary routes behind Explore',()=>{
  ...
  assert.match(indexHtml,/class="app-rail"/); assert.doesNotMatch(indexHtml,/data-view="insights" class="rail-nav-btn rail-secondary"|id="journalRailBtn"|id="settingsRailBtn"/); assert.match(indexHtml,/id="topMore"/); for(const id of ['insightsBtn','journalBtn','settingsBtn'])assert.match(indexHtml,new RegExp(`id="${id}"`)); assert.match(indexHtml,/data-open-logger/);
});
