...
test('shell foundation loads before Product Rebuild composition', () => { const design=indexHtml.indexOf('/css/design-system.css');const shell=indexHtml.indexOf('/css/navigation-shell.css');const rebuild=indexHtml.indexOf('/css/product-rebuild.css');assert.ok(design>=0&&shell>design&&rebuild>shell); });
...
test('secondary destinations are behind one accessible Explore control', () => { assert.match(indexHtml,/<details id="topMore" class="top-more">/);assert.match(indexHtml,/aria-label="Explore Insights, Journal and Settings"/);assert.match(indexHtml,/class="top-more-label">Explore/);for(const id of ['insightsBtn','journalBtn','settingsBtn'])assert.match(indexHtml,new RegExp(`id="${id}"`));assert.match(appJs,/function closeTopMore\(\)/);assert.match(appJs,/showView\('insights'\)/); });

test('mobile shell remains safe-area aware and touch sized', () => { assert.match(shellCss,/env\(safe-area-inset-bottom\)/);assert.match(rebuildCss,/\.bottom-nav \.nav-add\{[^}]*height:58px!important/s);assert.match(rebuildCss,/min-height:44px/); });

test('desktop shell owns Explore geometry and keeps secondary routes out of the rail', () => { const rail=between(indexHtml,'<aside class="app-rail"','</aside>');for(const view of ['today','plan','progress','wellness-boost'])assert.match(rail,new RegExp(`data-view="${view}"`));assert.doesNotMatch(rail,/data-view="insights"|id="journalRailBtn"|id="settingsRailBtn"/);assert.match(indexHtml,/id="topMore"/);assert.match(rail,/rail-log-btn[\s\S]*?Add activity/);assert.match(shellCss,/@media \(min-width:900px\)[\s\S]*\.topbar\{display:flex;position:fixed;z-index:95[\s\S]*\.top-actions\{display:flex\}/);assert.doesNotMatch(accessibilityCss,/@media\(min-width:900px\)[\s\S]*\.topbar/); });
