...
test('mobile secondary destinations are behind one accessible More control', () => { assert.match(indexHtml,/<details id="topMore" class="top-more">/);assert.match(indexHtml,/aria-label="More sections and settings"/);for(const id of ['insightsBtn','journalBtn','settingsBtn'])assert.match(indexHtml,new RegExp(`id="${id}"`));assert.match(rebuildCss,/\.topbar\{position:fixed!important/);assert.match(appJs,/function closeTopMore\(\)/);assert.match(appJs,/showView\('insights'\)/); });
...
test('desktop shell keeps primary routes persistent and secondary routes directly reachable', () => { const rail=between(indexHtml,'<aside class="app-rail"','</aside>');for(const view of ['today','plan','progress','wellness-boost','insights'])assert.match(rail,new RegExp(`data-view="${view}"`));assert.match(rail,/id="journalRailBtn"/);assert.match(rail,/id="settingsRailBtn"/);assert.match(rail,/rail-log-btn[\s\S]*?Add activity/);assert.match(currentCss,/\.app-rail\{position:fixed!important/); });

test('active primary navigation is semantic as well as visual', () => { const todayCurrent=indexHtml.match(/data-view="today"[^>]*aria-current="page"/g)||[];assert.equal(todayCurrent.length,2);assert.match(appJs,/setAttribute\('aria-current', 'page'\)/);assert.match(appJs,/removeAttribute\('aria-current'\)/); });
