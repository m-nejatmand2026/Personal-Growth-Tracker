...
test('Insights keeps evidence thresholds and association-only language',()=>{for(const threshold of ['0–6','7–20','21–41','42+'])assert.match(ui,new RegExp(threshold.replace('+','\\+')));assert.match(ui,/Still association, never causation/);assert.match(ui,/associated with/);assert.doesNotMatch(ui,/causes higher|causes lower|because of sleep/i)});
