...
test('Insights keeps evidence thresholds and association-only language',()=>{for(const threshold of ['0–6 days','7–20 days','21–41 days','42+ days'])assert.match(ui,new RegExp(threshold.replace('+','\\+')));assert.match(ui,/does not prove cause/);assert.match(ui,/associated with/);assert.match(ui,/how many observations support it/);assert.doesNotMatch(ui,/causes higher|causes lower|because of sleep/i)});
