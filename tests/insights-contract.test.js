...
test('Insights keeps evidence thresholds and association-only language',()=>{for(const threshold of ['0–6','7–20','21–41','42+'])assert.match(ui,new RegExp(threshold.replace('+','\\+')));assert.match(ui,/does not prove cause/);assert.match(ui,/association/i);assert.match(ui,/supporting observation counts/);assert.doesNotMatch(ui,/causes higher|causes lower|because of sleep/i)});
