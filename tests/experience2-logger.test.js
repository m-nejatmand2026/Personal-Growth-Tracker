...
test('release-blocking browser acceptance exercises Experience 2 Logger on desktop and 375px Chromium/WebKit',()=>{
  assert.match(browserRunner,/run_e2_suite\(\)/);
  assert.match(browserRunner,/GC_E2E_BASE_URL=http:\/\/127\.0\.0\.1:8787\/experience\/2\/ node --test "\$file"/);
  assert.match(browserRunner,/tests\/browser\/experience2-logger\.browser\.js/);
