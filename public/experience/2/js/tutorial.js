...
function card(step){const final=step.key==='done';return `<section class="gc-tutorial-card" role="dialog" aria-modal="false" aria-labelledby="gcTutorialTitle"><div class="gc-tutorial-top"><span>${final?'Complete':`Step ${Math.min(index+1,steps.length-1)} of ${steps.length-1}`}</span><button type="button" data-tutorial-skip>${final?'Close':'Close tutorial'}</button></div><h2 id="gcTutorialTitle">${step.title}</h2><p>${step.copy}</p>${progress()}<div class="gc-tutorial-actions">${index>0&&!final?'<button type="button" class="gc-tutorial-back" data-tutorial-back>Back</button>':''}<button type="button" class="gc-tutorial-next" data-tutorial-next>${step.button}</button></div></section>`;}
