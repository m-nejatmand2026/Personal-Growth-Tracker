...
function renderSignIn(status){
  ...
  const host=mountGate(`<section class="auth-shell"><div class="auth-intro"><div class="auth-brand"><span class="auth-brand-mark">◇</span><span>Growth Compass</span></div><div class="auth-intro-copy"><h1>Know what matters.<br>Know what to do next.</h1><p>Growth Compass connects the direction you want to move in with what you actually do — and helps you learn from what happens.</p></div><div class="auth-private-note"><span class="auth-private-dot"></span><span>Private workspace · Invite-only preview</span></div></div><div class="auth-card"><div class="auth-card-head"><h2>Welcome</h2><p>Sign in to your private Growth Compass workspace.</p></div><div class="auth-provider-stack">${providerButtons}</div>${emailBlock}<p class="auth-message" id="authMessage" aria-live="polite"></p><p class="auth-invite-note">This preview is invite-only. Use the same email address that received your invitation.</p></div></section>`);
