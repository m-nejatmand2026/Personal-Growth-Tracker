const CACHE='growth-compass-preview2-e1-v1';
const CORE=['/experience/1/','/styles.css','/experience/1/manifest.webmanifest','/icon.svg'];
self.addEventListener('install',event=>event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(CORE)).then(()=>self.skipWaiting())));
self.addEventListener('activate',event=>event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(key=>key.startsWith('growth-compass-preview2-e1-')&&key!==CACHE).map(key=>caches.delete(key)))).then(()=>self.clients.claim())));
