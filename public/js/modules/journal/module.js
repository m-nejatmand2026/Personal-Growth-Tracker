...
export const journalModule=Object.freeze({
  ...
  renderView({model}){return `<section class="journal-hero"><div><h2>Journal</h2><p class="gc-sr-only">Private reflection without streaks, points or optimization pressure.</p></div><button type="button" class="journal-new" data-journal-write="${model.date}">＋ New entry</button></section>
    <form id="journalSearchForm" class="journal-search"><label class="journal-search-main"><span class="gc-sr-only">Search</span><input id="journalSearch" value="${escapeHtml(model.query||'')}" placeholder="Search your writing"></label><button type="submit">Search</button>${(model.query||model.filterDate)?'<button type="button" id="journalClearFilters">Clear</button>':''}<details class="journal-filter-disclosure" ${model.filterDate?'open':''}><summary>Filter by date</summary><label><span class="gc-sr-only">Date</span><input id="journalFilterDate" type="date" value="${escapeHtml(model.filterDate||'')}"></label></details></form>
    ...
    <section class="journal-list-section"><div class="os-section-head"><div><h2>${model.items.length?`${model.items.length} reflection${model.items.length===1?'':'s'}`:'Nothing here yet'}</h2></div><small>${model.filterDate?escapeHtml(model.filterDate):'Recent 90 days'}</small></div><div class="journal-entry-list">${model.items.length?model.items.map(entryHtml).join(''):'<div class="journal-empty"><strong>Your journal is empty for this view.</strong><span>Write something, change the date filter, or clear search.</span></div>'}</div></section>`},
