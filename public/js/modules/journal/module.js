...
export const journalModule=Object.freeze({
  ...
  renderView({model}){return `<!-- class="journal-hero journal-action-hero" --><section class="journal-hero journal-action-hero living-journal-hero"><div class="living-page-heading"><h2>Your Reflections</h2><p>Tracing the threads of your growth.</p></div><div class="living-reflection-stats"><div><strong>${model.items.length}</strong><span>Total</span></div><div><strong>${reflectionStreak(model.items)}</strong><span>Streak</span></div></div><button type="button" class="journal-new" data-journal-write="${model.date}">＋ New reflection</button></section>
    <form id="journalSearchForm" class="journal-search"><label class="journal-search-main"><span aria-hidden="true">⌕</span><span class="gc-sr-only">Search</span><input id="journalSearch" value="${escapeHtml(model.query||'')}" aria-label="Search your writing" placeholder="Search memories, keywords"></label><button type="submit">Search</button>${(model.query||model.filterDate)?'<button type="button" id="journalClearFilters">Clear</button>':''}<details class="journal-filter-disclosure" ${model.filterDate?'open':''}><summary>Filter</summary><label><span class="gc-sr-only">Date</span><input id="journalFilterDate" type="date" value="${escapeHtml(model.filterDate||'')}"></label></details></form>
    <details class="journal-trust-disclosure"><summary>About your journal</summary><div><strong>No journaling streaks.</strong><span>Missing a day is not a failure. Journal content is excluded from Insights and AI unless a future explicit permission says otherwise.</span><span class="gc-sr-only">Journal text is not used by Progress, Insights or AI in this beta.</span></div></details>
    <section class="journal-list-section"><div class="os-section-head"><div><h2>${model.items.length?'This Week':'A quiet beginning'}</h2></div><small>${model.filterDate?escapeHtml(model.filterDate):'Recent 90 days'}</small></div><div class="journal-entry-list">${model.items.length?model.items.map(entryHtml).join(''):'<div class="journal-empty"><strong>Your reflections will gather here.</strong><span>Write when there is something you want to remember.</span></div>'}</div></section>`},
