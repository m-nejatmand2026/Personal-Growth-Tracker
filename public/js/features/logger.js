...
function fallbackActivities() {
  ...
  for (const x of state.data.sessions || []) {
    ...
    items.push({
      key: x.activity_key,
      name: x.activity_name || x.activity_key
    });
  }
...
function repeatButtons(items) {
  if (items == null) {
    return '<div class="logger-loading">Loading recent repeats…</div>';
  }

  if (!items.length) {
    return '<div class="logger-empty">Your recent activity combinations will appear here.</div>';
  }
...
async function loadRecentRepeats() {
  ...
  try {
    const response = await api(
      `/api/v1/progress?from=${from}&to=${state.date}&limit=100`
    );
...
export function createLogger({ onSaved, onIntent }) {
  ...
  function activityOptions(selected = '') {
    if (!activities.length) {
      return '<option value="">No activities available yet</option>';
    }
  ...
  function currentMode() {
    return host.querySelector(
      'input[name="loggerEntryMode"]:checked'
    )?.value || 'done';
  }
  ...
  function updateModeUi() {
    ...
    if ($('#loggerSaveButton')) {
      $('#loggerSaveButton').textContent = copy.button;
    }

    if ($('#loggerModeHint')) {
      $('#loggerModeHint').textContent = copy.hint;
    }
  }
  ...
  function render(prefill = {}) {
    ...
    host.innerHTML = `
      ...
      <section class="logger-panel" role="dialog" aria-modal="true" aria-labelledby="loggerTitle" tabindex="-1">
        ...
        <form id="loggerForm" class="logger-form">
          ...
          <label class="logger-field">
            <span>Subtype / focus <small>optional</small></span>
            <input id="loggerSubtype" maxlength="80" placeholder="${escapeHtml(subtypePlaceholder())}">
          </label>
          ...
          <label class="logger-field">
            <span>Date</span>
            <input id="loggerDate" type="date" value="${escapeHtml(date)}" required>
          </label>
    ...
    host.querySelectorAll('[data-logger-close]').forEach((item) => {
    ...
    host.querySelectorAll('input[name="loggerEntryMode"]').forEach((item) => {
      item.addEventListener('change', updateModeUi);
    });
    ...
    $('#loggerForm')?.addEventListener('submit', async (event) => {
      ...
      if (!Number.isInteger(minutes) || minutes < 1 || minutes > 1440) {
        return toast('Duration must be 1–1440 minutes');
      }
      if (!occurredOn) return toast('Choose a date');
      if (entryMode === 'in_progress' && occurredOn !== state.date) {
        return toast('Doing now must use Today');
      }
