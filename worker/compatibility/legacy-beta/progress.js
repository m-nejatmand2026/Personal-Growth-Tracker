...
import { activitiesContractV1 } from '../../modules/activities/public.js';
import { plansContractV1 } from '../../modules/plans/public.js';
...
/**
 * Temporary /api/week response-shape adapter.
 * Minimum/Target come from canonical Plans; Actual comes from Progress.
 */
export async function getLegacyWeek(DB, profileId, start) {
  ...
  for (const item of history) {
    ...
    actualByGoal.set(goalId, (actualByGoal.get(goalId) || 0) + Math.max(0, Number(item.minutes) || 0));
  }
  ...
  for (const date of datesBetween(start, end)) {
    ...
    for (const candidate of allocationModel.versions) {
      if (candidate.effective_from <= date && (!candidate.effective_to || candidate.effective_to >= date)) {
        version = candidate;
      }
    }
    if (!version) continue;
    ...
    for (const value of version.values) {
      ...
      current.target_minutes += dailyShare(value.time_target_minutes, value.period, date);
      current.minimum_minutes += dailyShare(value.time_minimum_minutes, value.period, date);
