...
import { createArea, getArea, getAreaTemplate, listAreas, listAreaTemplates, updateArea } from '../data/areas.js';
...
async function normalizeAreaInput(DB, body, existing = null) {
  const templateKeyRaw = body.template_key ?? existing?.template_key ?? null;
  const templateKey = typeof templateKeyRaw === 'string' && templateKeyRaw ? templateKeyRaw : null;
  const template = templateKey ? await getAreaTemplate(DB, templateKey) : null;
  if (templateKey && !template) return { error: 'Area template not found.' };

  const name = requiredText(body.name ?? existing?.name ?? template?.name, 80);
  if (!name) return { error: 'Area name is required and must be 80 characters or fewer.' };
  const iconRaw = body.icon ?? existing?.icon ?? template?.icon ?? null;
  const colorRaw = body.color ?? existing?.color ?? template?.default_color ?? null;
  const color = safeColor(colorRaw);
  if (colorRaw && !color) return { error: 'Area color must be a six-digit hex color such as #0F766E.' };
  const sortOrder = body.sort_order ?? existing?.sort_order ?? template?.sort_order ?? 100;
  const active = body.active ?? (existing ? Boolean(existing.active) : true);

  return {
    value: {
      name,
      icon: typeof iconRaw === 'string' && /^[a-z0-9-]{1,50}$/i.test(iconRaw) ? iconRaw : null,
      color,
...
export async function createAreaRoute({ request, env }) {
  ...
  const normalized = await normalizeAreaInput(env.DB, body);
...
export async function updateAreaRoute({ request, url, env }) {
  ...
  const body = await readJsonBody(request);
  const normalized = await normalizeAreaInput(env.DB, body, existing);
  if (normalized.error) return bad(normalized.error);
