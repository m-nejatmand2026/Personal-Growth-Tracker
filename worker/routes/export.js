...
export async function exportRoute({ request, env }) {
  ...
  const [
    activities, targets, sessions, energy, lessons, roadmap, settings,
    areas, goals, goalActivities, planVersions, goalPlanValues,
    commitments, progressRecords, sleepLogs, contextLogs
  ] = await Promise.all([
    ...
    env.DB.prepare(`
      SELECT gpv.* FROM goal_plan_values gpv
      JOIN plan_versions pv ON pv.id=gpv.plan_version_id
      WHERE pv.profile_id=? ORDER BY pv.effective_from,gpv.goal_id
    `).bind(profileId).all(),
  ...
  return json({
    ...
    version_one: {
      areas: areas.results,
      goals: goals.results,
      goal_activities: goalActivities.results,
      plan_versions: planVersions.results,
      goal_plan_values: goalPlanValues.results,
      capacity_commitments: commitments.results,
      progress_records: progressRecords.results,
      sleep_logs: sleepLogs.results,
      day_context_logs: contextLogs.results
    },
    legacy_beta: {
