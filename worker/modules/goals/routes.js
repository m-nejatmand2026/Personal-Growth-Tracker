...
import { areasContractV1 } from '../areas/public.js';
import { createGoal, deleteGoal, getGoal, listGoals, updateGoal } from './data.js';
...
function withAreaReference(goal, area) {
  if (!goal) return null;
  return {
    ...goal,
    area_name: area?.name || null,
    area_color: area?.color || null,
    area_icon: area?.icon || null
  };
}

async function enrichGoal(DB, profileId, goal) {
  if (!goal) return null;
  if (goal.area_id == null) return withAreaReference(goal, null);
  const area = await areasContractV1.getReference(DB, profileId, goal.area_id);
  return withAreaReference(goal, area);
}

async function enrichGoals(DB, profileId, goals) {
  const areas = await areasContractV1.listReferences(DB, profileId, { includeArchived: true });
  const areaById = new Map(areas.map((area) => [Number(area.id), area]));
  return goals.map((goal) => withAreaReference(goal, goal.area_id == null ? null : areaById.get(Number(goal.area_id))));
}
...
async function normalizeGoalInput(DB, profileId, body, existing = null) {
  ...
  if (areaId != null) {
    ...
    const area = await areasContractV1.getReference(DB, profileId, areaId);
  ...
  const sortOrderRaw = body.sort_order ?? existing?.sort_order;
  return {
    value: {
      area_id: areaId,
      name,
      ...
      priority,
      status,
      sort_order: Number.isFinite(Number(sortOrderRaw)) ? Math.round(Number(sortOrderRaw)) : 100
    }
  };
}
...
export async function listGoalsRoute({ request, url, env }) {
  ...
  const goals = await listGoals(env.DB, profileId, includeArchived);
  return json({ items: await enrichGoals(env.DB, profileId, goals) });
}
...
export async function createGoalRoute({ request, env }) {
  ...
  const goal = await createGoal(env.DB, profileId, normalized.value);
  return json({ item: await enrichGoal(env.DB, profileId, goal) }, 201);
}
...
export async function updateGoalRoute({ request, url, env }) {
  ...
  if (normalized.error) return bad(normalized.error);
  const goal = await updateGoal(env.DB, profileId, id, normalized.value);
  return json({ item: await enrichGoal(env.DB, profileId, goal) });
}
...
export async function archiveGoalRoute({ request, url, env }) {
  ...
  const existing = await getGoal(env.DB, profileId, id);
  if (!existing) return bad('Goal not found.', 404);
  const goal = await updateGoal(env.DB, profileId, id, { ...existing, status: 'archived' });
  return json({ item: await enrichGoal(env.DB, profileId, goal) });
}

export async function removeGoalRoute({ request, url, env }) {
  const profileId = resolveProfileId(request);
  ...
  const existing = await getGoal(env.DB, profileId, id);
  if (!existing) return bad('Goal not found.', 404);
  await deleteGoal(env.DB, profileId, id);
