...
function normalizeCommitmentInput(body, existing = null) {
  ...
  let minutes = integerInRange(body.minutes ?? existing?.minutes, 0, 1440);
  ...
  const rawStart = body.start_time !== undefined ? body.start_time : existing?.start_time;
  const rawEnd = body.end_time !== undefined ? body.end_time : existing?.end_time;
  const startTime = rawStart == null || rawStart === '' ? null : String(rawStart);
  const endTime = rawEnd == null || rawEnd === '' ? null : String(rawEnd);
  if (Boolean(startTime) !== Boolean(endTime)) return { error: 'Choose both a start time and an end time, or leave both blank.' };
  if (startTime && (timeToMinutes(startTime) == null || timeToMinutes(endTime) == null)) {
    return { error: 'Routine times must use HH:MM.' };
  }
  if (startTime) {
    const exactMinutes = clockDuration(startTime, endTime);
    if (!exactMinutes) return { error: 'Start and end time must describe a real time block.' };
    if (dailyResult.value) return { error: 'Exact clock blocks cannot also use per-day duration overrides.' };
  ...
  const requestedFlexibility = body.flexibility
    ?? existing?.flexibility
    ?? ((body.protected ?? (existing ? Boolean(existing.protected) : true)) ? 'fixed' : 'flexible');
  if (!FLEXIBILITY.has(requestedFlexibility)) return { error: 'flexibility must be fixed, preferred, or flexible.' };
  const protectedValue = body.protected !== undefined
    ? Boolean(body.protected)
    : requestedFlexibility === 'fixed';
...
export async function updateCapacityCommitmentRoute({ request, url, env }) {
  ...
  const existing = await getCapacityCommitment(env.DB, profileId, id);
  ...
  if (!versionFrom) return json({ item: await updateCapacityCommitment(env.DB, profileId, id, normalized.value) });
  if (!isDateKey(versionFrom)) return bad('Invalid version_from date.');
  if (existing.effective_to && versionFrom > existing.effective_to) return bad('version_from falls after this schedule version ends.');
  if (existing.effective_from && versionFrom < existing.effective_from) return bad('Backdating before the current schedule version is not supported in this editor.');
