...
function parseDailyMinutes(raw) {
  ...
  try {
    ...
    return Array.isArray(value) && value.length === 7 ? value.map((item) => Number(item) || 0) : null;
  } catch { return null; }
}

function hydrateCommitment(row) {
  if (!row) return row;
  return { ...row, daily_minutes: parseDailyMinutes(row.daily_minutes_json) };
}

export async function getCapacityCommitment(DB, profileId, id) {
  return hydrateCommitment(await DB.prepare(`
    SELECT * FROM capacity_commitments WHERE id=? AND profile_id=?
  `).bind(id, profileId).first());
}

export async function listCapacityCommitments(DB, profileId, includeInactive = false, activeOn = null) {
  const filters = ['profile_id=?'];
  const bindings = [profileId];
  if (!includeInactive) filters.push('active=1');
  if (activeOn) {
    filters.push('(effective_from IS NULL OR effective_from<=?)');
    filters.push('(effective_to IS NULL OR effective_to>=?)');
    bindings.push(activeOn, activeOn);
  }
  ...
  `).bind(...bindings).all();
  return results.map(hydrateCommitment);
}
...
export async function getPlannedGoalMinutes(DB, profileId, dateText, period) {
  ...
  const allocationModel = await plansContractV1.getActiveAllocationsForRange(DB, profileId, bounds.start, bounds.end);
  const versions = allocationModel.versions;
  ...
  for (const date of dates) {
    ...
    for (const version of versions) {
      if (version.effective_from <= date && (!version.effective_to || version.effective_to >= date)) activeVersion = version;
    }
    if (!activeVersion) continue;
    usedPlanVersions.add(activeVersion.id);
    for (const value of activeVersion.values) {
      const current = goalTotals.get(value.goal_id) || {
        goal_id: value.goal_id,
        name: value.goal_name,
        period_target_minutes: 0,
        period_minimum_minutes: 0
      };
      current.period_target_minutes += dailyShare(value.time_target_minutes, value.period, date);
      current.period_minimum_minutes += dailyShare(value.time_minimum_minutes, value.period, date);
      goalTotals.set(value.goal_id, current);
    }
  }

  const goals = [...goalTotals.values()].map((goal) => ({
    ...goal,
...
function commitmentBindings(profileId, input, seriesId) {
  return [
    profileId,
    input.kind,
    input.name,
    ...
    seriesId,
    input.daily_minutes ? JSON.stringify(input.daily_minutes) : null,
    input.start_time || null,
    input.end_time || null,
    input.flexibility || (input.protected === false ? 'flexible' : 'fixed')
  ];
}
...
export async function createCapacityCommitment(DB, profileId, input) {
  ...
  const result = await DB.prepare(`
    INSERT INTO capacity_commitments(
      profile_id,kind,name,minutes,weekday_mask,effective_from,effective_to,
      protected,active,sort_order,series_id,daily_minutes_json,start_time,end_time,flexibility
    ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
  `).bind(...commitmentBindings(profileId, input, seriesId)).run();
  return getCapacityCommitment(DB, profileId, result.meta.last_row_id);
}
...
export async function updateCapacityCommitment(DB, profileId, id, input) {
  await DB.prepare(`
    ...
    SET kind=?,name=?,minutes=?,weekday_mask=?,effective_from=?,effective_to=?,
        protected=?,active=?,sort_order=?,daily_minutes_json=?,start_time=?,end_time=?,flexibility=?,
...
export async function versionCapacityCommitment(DB, profileId, existing, input, versionFrom, previousDate) {
  ...
  `).bind(...values);
  const closePrevious = DB.prepare(`
    UPDATE capacity_commitments
    SET series_id=?, effective_to=?, updated_at=CURRENT_TIMESTAMP
    WHERE id=? AND profile_id=?
  `).bind(seriesId, previousDate, existing.id, profileId);
  const results = await DB.batch([closePrevious, insert]);
  const insertedId = results[1]?.meta?.last_row_id;
  if (insertedId) return getCapacityCommitment(DB, profileId, insertedId);
  return hydrateCommitment(await DB.prepare(`
    SELECT * FROM capacity_commitments
    WHERE profile_id=? AND series_id=? AND effective_from=?
    ORDER BY id DESC LIMIT 1
  `).bind(profileId, seriesId, versionFrom).first());
}

export async function exportCapacityData(DB, profileId) {
  const { results } = await DB.prepare(`
    SELECT * FROM capacity_commitments WHERE profile_id=? ORDER BY sort_order,id
  `).bind(profileId).all();
  return results;
}
