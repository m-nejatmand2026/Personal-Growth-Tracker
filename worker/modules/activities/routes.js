...
async function normalizeActivityInput(
  DB,
  profileId,
  body,
  existing = null
  ...
  const goal = await goalsContractV1.getReference(
    DB,
    profileId,
    goalId
  );
  ...
  if (goal.status === 'archived') {
    return {
      error: 'An Activity cannot belong to an archived Goal.'
    };
  }
  ...
  if (!name) {
    return {
      error:
        'Activity name is required and must be 120 characters or fewer.'
    };
  }
  ...
  const description =
    descriptionRaw == null
      ? null
      : optionalText(descriptionRaw, 1000);

  if (descriptionRaw != null && description == null) {
    return {
...
export async function listActivitiesRoute({
  request,
  url,
  env
}) {
  ...
  // Ask the declared Goals public contract which Goal references are usable.
  const goals = await goalsContractV1.listReferences(
    env.DB,
    profileId
  );

  const usableGoalIds = new Set(
    goals.map((goal) => Number(goal.id))
  );

  return json({
    items: items.filter(
      (item) => usableGoalIds.has(Number(item.goal_id))
    )
  });
}

export async function createActivityRoute({
  request,
  env
}) {
  const profileId = resolveProfileId(request);
...
export async function updateActivityRoute({
  ...
  if (!Number.isInteger(id) || id <= 0) {
    return bad('Invalid activity id.');
  }
  ...
  if (!existing) {
    return bad('Activity not found.', 404);
  }
...
export async function archiveActivityRoute({
  request,
  url,
  env
}) {
