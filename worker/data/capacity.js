...
function hydrateCommitment(row) {
  ...
  return {
    ...row,
    daily_minutes: parseDailyMinutes(row.daily_minutes_json)
  };
}
...
export async function listCapacityCommitments(DB, profileId, includeInactive = false, activeOn = null) {
  ...
  const query = `SELECT * FROM capacity_commitments WHERE ${filters.join(' AND ')} ORDER BY active DESC,sort_order,name,id`;
...
export async function createCapacityCommitment(DB, profileId, input) {
  ...
  const result = await DB.prepare(`
    INSERT INTO capacity_commitments(
      profile_id,kind,name,minutes,weekday_mask,effective_from,effective_to,protected,active,sort_order,series_id,daily_minutes_json
    ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)
  `).bind(
    profileId,
    input.kind,
  ...
  return hydrateCommitment(await DB.prepare('SELECT * FROM capacity_commitments WHERE id=? AND profile_id=?')
...
export async function updateCapacityCommitment(DB, profileId, id, input) {
  await DB.prepare(`
    ...
    SET kind=?,name=?,minutes=?,weekday_mask=?,effective_from=?,effective_to=?,
        protected=?,active=?,sort_order=?,daily_minutes_json=?,updated_at=CURRENT_TIMESTAMP
  ...
  return hydrateCommitment(await DB.prepare('SELECT * FROM capacity_commitments WHERE id=? AND profile_id=?').bind(id, profileId).first());
}
...
export async function versionCapacityCommitment(DB, profileId, existing, input, versionFrom, previousDate) {
  ...
  if (insertedId) {
    return hydrateCommitment(await DB.prepare('SELECT * FROM capacity_commitments WHERE id=? AND profile_id=?')
      .bind(insertedId, profileId).first());
  }
